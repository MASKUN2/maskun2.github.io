...
:   Avro compares schema versions by looking at a writer schema (origin payload) and a reader schema (your application payload). See the [Avro documentation](https://avro.apache.org/docs/1.11.1/spec.html) for more information.
...
You can customize the schema storage by using the [Spring Boot SQL database and JDBC configuration options](https://docs.spring.io/spring-boot/docs/current-SNAPSHOT/reference/htmlsingle/#boot-features-sql).
...
When reading messages that contain version information (that is, a `contentType` header with a scheme like the one described under `Schema Registration Process (Serialization)`, the converter queries the Schema server to fetch the writer schema of the message.
