...
* If none of the Serdes provided by Kafka Streams don’t match the types, then it will use JsonSerde provided by Spring Kafka. In this case, the binder assumes that the types are JSON friendly.
  ...
  Before falling back to the `JacksonJsonSerde` though, the binder checks at the default `Serde` s set in the Kafka Streams configuration to see if it is a `Serde` that it can match with the incoming KStream’s types.
...
If that doesnt’t work, then it falls back to `JacksonJsonSerde` provided by the Spring Kafka project, but first look at the default `Serde` configuration to see if there is a match.
...
If Serde inference fails, and no binding level Serdes are provided, then the binder falls back to the `JacksonJsonSerde`, but look at the default Serdes for a match.
