...
Since Spring Cloud Stream v2.1, another alternative for defining *stream handlers* and *sources* is to use built-in
...
Please see [Functional Binding Names](functional-binding-names.html#functional-binding-names) section for details on naming convention used to establish binding names.
...
To learn how to customize the polling mechanism, see [Polling Configuration Properties](#polling-configuration-properties) section.
...
subsequent reuse (see [StreamBridge and Dynamic Destinations](#streambridge-and-dynamic-destinations) for more details).
...
|  |  |
| --- | --- |
|  | Unlike it is explained in [Explicit Binding Creation](explicit-binding-creation.html), *StreamBridge* was designed with both performance in mind and ability to create as many bindings on the fly as necessary. To achieve that the actual bindings created by the *StreamBridge* are not cached in the application context and therefore can not be managed as explained in [Binding visualization and control](binding_visualization_control.html). However if it is still your desire to use *StreamBridge* to create bindings dynamically and manage them after, please use the following mechanism to create binding explicitly before using *StreamBridge* - ref:spring-cloud-stream/binding\_visualization\_control.adocl#\_define\_new\_and\_manage\_existing\_bindings[Define New Binding Programmatically] |

#### StreamBridge with async send

`StreamBridge` uses sending mechanism provided by *Spring Integration* framework which is at the core of the *Spring Cloud Stream*. By default this mechanism uses the senderâs thread. In other words, the send is blocking. While this is ok for many cases, there are cases when you want such send to be async. To do that use `setAsync(true)` method of the `StreamBridge` before invoking one of the send methods.
...
described in [Routing FROM consumer](event-routing.html#routing-from-consumer) section.
...
as described in [Routing FROM consumer](event-routing.html#routing-from-consumer) section.
...
|  |  |
| --- | --- |
|  | If you are using the Solace PubSub+ binder, Spring Cloud Stream has reserved the `scst_targetDestination` header (retrievable via BinderHeaders.TARGET\_DESTINATION), which allows for messages to be redirected from their bindings' configured destination to the target destination specified by this header. This allows for the binder to manage the resources necessary to publish to dynamic destinations, relieving the framework from having to do so, and avoids the caching issues mentioned in the previous Note. More info [here](https://github.com/SolaceProducts/solace-spring-cloud/tree/master/solace-spring-cloud-starters/solace-spring-cloud-stream-starter#dynamic-producer-destinations). |
...
For more information on multiple binders scenarios, please see [Binders](binders.html#binders) section and specifically [Multiple Binders on the Classpath](multiple-binders.html#multiple-binders)
...
Since *Spring Cloud Function* is build on top of [Project Reactor](https://projectreactor.io/) there isn’t much you need to do
...
and modify the `spring.cloud.function.definition` property to reflect your intention to compose a new function from both âtoUpperCaseâ and âwrapInQuotesâ.
...
feature described in [Functional Binding Names](functional-binding-names.html#functional-binding-names) section can help.
...
* *Big Data: Imagine the source of data you’re dealing with is highly un-organized and contains various types of data elements
  (e.g., orders, transactions etc) and you effectively need to sort it out.*
...
multiple *streams* of data. And that is the type of use cases we are targeting here.
...
abstractions provided by [Project Reactor](https://projectreactor.io/) (i.e., `Flux` and `Mono`) which is already available on the
...
provided by *Project Reactor* - Tuples. However, we are working on allowing a more flexible signatures.

|  |  |
| --- | --- |
|  | Please refer to [Binding and Binding names](binding-names.html#binding-names) section to understand the naming convention used to establish *binding names* used by such application. |
...
|  |  |
| --- | --- |
|  | As with functions with multiple inputs/outputs, please refer to [[Binding and Binding names]](#Binding and Binding names) section to understand the naming convention used to establish *binding names* used by such application. |
...
If implemented, this interface is called by the frameworkâs message converter logic to perform post-processing on the batched message when a particular payload cannot be converted.
...
of [Enterprise Integration Patterns](https://www.enterpriseintegrationpatterns.com) (EIP). These are best handled by using a
framework such as [Spring Integration](https://spring.io/projects/spring-integration) (SI), which is a reference implementation of EIP.

Thankfully SI already provides support for exposing integration flows as functions via
[Integration flow as gateway](https://docs.spring.io/spring-integration/docs/current/reference/html/#java-dsl-gateway)
Consider the following sample:
...
See [Content Type Negotiation](content-type.html) section for more on type conversion.
...
|  |  |
| --- | --- |
|  | Please refer to [[Binding and Binding names]](#Binding and Binding names) section to understand the naming convention used to establish *binding names* used by such application. |
...
you may find [this post](https://spring.io/blog/2019/10/25/spring-cloud-stream-and-spring-integration) very interesting, as it dives a bit deeper
