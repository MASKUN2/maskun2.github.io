...
:   The default binder to use, if multiple binders are configured.
    See [Multiple Binders on the Classpath](multiple-binders.html).
...
:   This property is only applicable when the `cloud` profile is active and Spring Cloud Connectors are provided with the application.
    ...
    The typical usage of this property is to be nested in a customized environment [when connecting to multiple systems](multiple-systems.html).
