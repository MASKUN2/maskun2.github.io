...
If `resetOffsets` is `false`, normal Kafka consumer [`auto.offset.reset`](https://kafka.apache.org/documentation/#consumerconfigs_auto.offset.reset) semantics apply.
...
See the [`offsets.retention.minutes`](https://kafka.apache.org/documentation/#brokerconfigs_offsets.retention.minutes) broker property for more information.
...
For more control over topic offsets, see [rebalance listener](rebalance_listener.html); when a listener is provided, `resetOffsets` should not be set to `true`, otherwise, that will cause an error.