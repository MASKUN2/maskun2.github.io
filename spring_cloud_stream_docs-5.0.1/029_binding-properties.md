...
there are two bindings named `uppercase-in-0` for input and `uppercase-out-0` for output. See [[Binding and Binding names]](#Binding and Binding names) for more details.
...
:   The consumer group of the binding.
    ...
    See [Consumer Groups](consumer-groups.html).
...
:   The binder used by this binding.
    See `Multiple Binders on the Classpath` for details.
...
:   When set to a value greater than equal to zero, it allows customizing the instance count of this consumer (if different from `spring.cloud.stream.instanceCount`).
    ...
    See `Instance Index and Instance Count` for more information.
