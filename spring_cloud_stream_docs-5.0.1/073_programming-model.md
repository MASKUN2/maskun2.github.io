...
When using the programming model provided by Kafka Streams binder, both the high-level [Streams DSL](https://docs.confluent.io/current/streams/developer-guide/dsl-api.html) and a mix of both the higher level and the lower level [Processor-API](https://docs.confluent.io/current/streams/developer-guide/processor-api.html) can be used as options.
