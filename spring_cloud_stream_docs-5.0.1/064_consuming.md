...
However, in this case, you need to provide a custom implementation for a [RecordMessageConverter](https://docs.spring.io/spring-kafka/docs/current/api/org/springframework/kafka/support/converter/RecordMessageConverter.html).
By default, the reactive Kafka binder uses a [MessagingMessageConverter](https://docs.spring.io/spring-kafka/docs/current/api/org/springframework/kafka/support/converter/MessagingMessageConverter.html) that converts the payload and headers from the `ConsumerRecord`.
