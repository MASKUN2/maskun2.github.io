...
1. `JsonMessageConverter`: As the name suggests it supports conversion of the payload of the `Message` to/from POJO for cases when `contentType` is `application/json` (DEFAULT).
...
3. `ObjectStringMessageConverter`: Supports conversion of any type to a `String` when `contentType` is `text/plain`.
   It invokes Objectâs `toString()` method or, if the payload is `byte[]`, a new `String(byte[])`.
...
does not know how to convert. If that is the case, you can add custom `MessageConverter`. See [User-defined Message Converters](overview-user-defined-message-converters.html).