...
Normally, if a producer binding has `batch-enabled=true` (see [Rabbit Producer Properties](prod-props.html)), or a message is created by a `BatchingRabbitTemplate`, elements of the batch are returned as individual calls to the listener method.
