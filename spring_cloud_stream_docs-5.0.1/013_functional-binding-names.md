...
so it’s only relevant for [Functions with multiple input and output arguments](producing-and-consuming-messages.html#functions_with_multiple_input_and_output_arguments).

|  |  |
| --- | --- |
|  | For [Explicit Binding Names](explicit-binding-creation.html), the above naming convention is not necessary (see [Explicit Binding Creation](explicit-binding-creation.html)) |
