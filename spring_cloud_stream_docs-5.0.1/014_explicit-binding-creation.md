...
information please see [Define new and manage existing bindings](binding_visualization_control.html#_define_new_and_manage_existing_bindings)
...
|  |  |
| --- | --- |
|  | Unlike with [Functional Binding Names](functional-binding-names.html), explicitly created bindings do not require additional suffix such as `in-0/out-0` given that: |

* the binding name is not backed by a function
* it already represents *input* or *output* binding by definition
