...
See “[Error Handling](../../spring-cloud-stream/overview-error-handling.html)” for more information.
...
As well as enabling producer error channels (as described in “[Error Handling](../../spring-cloud-stream/overview-error-handling.html)”), the RabbitMQ binder only sends messages to the channels if the connection factory is appropriately configured, as follows:
...
Also see [Publisher Confirms](publisher-confirms.html) for an alternative mechanism to receive returned messages.
...
There is no automatic handling of these exceptions (such as sending to a [dead-letter queue](../rabbit_dlq.html)).
