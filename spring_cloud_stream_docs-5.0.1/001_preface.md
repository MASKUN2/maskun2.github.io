...
2. [Importing the Project into Your IDE](#spring-cloud-stream-preface-importing-project)
3. [Adding a Message Handler, Building, and Running](#spring-cloud-stream-preface-adding-message-handler)
...
* We are using functional programming model (see [[Spring Cloud Function support]](#Spring Cloud Function support)) to define a single message handler as `Consumer`.
...
From the broker, the message arrives in a form of a byte[]. It is then transformed to a `Message<byte[]>` by the binders whereas you can see the payload of the message maintains its raw form. The headers of the message are `<String, Object>`, where values are typically another primitive or a collection/array of primitives, hence Object.
...
Here is one example to demonstrate the problem; Imagine you have a routing expression to route to different functions based on payload type. This requirement would imply payload conversion from byte[] to a specific type and then applying the SpEL. However, in order to perform such conversion we would need to know the actual type to pass to converter and that comes from function’s signature which we donât know which one. A better approach to solve this requirement would be to pass the type information as message headers (e.g., `application/json;type=foo.bar.Baz` ). Youâll get a clear readable String value that could be accessed and evaluated in a year and easy to read SpEL expression.
