...
*a) route events to a particular event subscriber* or
*b) route events produced by an event subscriber to a particular destination*.
Here we’ll refer to it as route âTOâ and route âFROMâ.
...
|  |  |
| --- | --- |
|  | For the purposes of binding the name of the routing destination is `functionRouter-in-0` (see RoutingFunction.FUNCTION\_NAME and binding naming convention [Functional Binding Names](functional-binding-names.html)). |
...
such message will be routed to the appropriate (âevenâ or âoddâ) Consumer.
...
Also, please see [Using StreamBridge](producing-and-consuming-messages.html#sending-arbitrary-data-to-an-output-e-g-foreign-event-driven-sources) section to see how yet another option can be utilized for similar cases.