...
### 도입의 과정
테스트는 각 테스트 메소드가 독립적인 환경에서 실행되어야한다. 
...
테스트 범위에 데이터 베이스가 있는 경우 따로 테스트 컨테이너를 두지 않는다면 잔존 데이터의 영향을 받게 되므로 나는 각 테스트마다 로컬 테스트 데이터베이스의 Trancate를 실시했다.
그리고 @BeforeEach 메소드에서 jdbcTemplate 을 사용하여 테스트 목적에 
